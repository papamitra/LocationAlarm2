...
class SetAlarm extends PreferenceActivity with ActivityResultTrait {
  ...
  private lazy val alarmTable = new AlarmTable(this)
  ...
  override def onCreate(saveInstanceState: Bundle) {
    ...
    alarmTable.findAll(alarmTable._id === id).headOption match {
      case Some(alarm) =>
        ...
        mLabel withActions (_ setText alarm.label.is,
          _ setSummary alarm.label.is)
        ...
        mInitialized = alarm.initialized.is
      case _ =>
    ...
    mLocation.setOnPreferenceClickListener(
      new OnPreferenceClickListener() {
        override def onPreferenceClick(pref: Preference): Boolean = {
          val mapIntent = new Intent(SetAlarm.this, classOf[LocationPicker]) withActions (_ putExtra (Alarms.ALARM_INTENT_INITIALIZED, mInitialized),
  ...
  def saveAlarm(id: Int) {
    ...
    alarm._id(id).
      ...
      shour(mStartHour).
      sminute(mStartMinute).
      min(mMinList.getValue.toInt)
